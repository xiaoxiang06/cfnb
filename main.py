...
支持 Windows / Linux
优化：新增 URL 优选、本地 ipv4.txt、ipv4.csv（万能正则适配各类格式）三大模式混合加载
"""
...
def load_config():
    ...
    defaults = {
        "SOURCE_URL_MODE_ENABLED": True,
        "SOURCE_TXT_MODE_ENABLED": True,
        "SOURCE_TXT_FILE": "ipv4.txt",
        "SOURCE_CSV_MODE_ENABLED": True,
        "SOURCE_CSV_FILE": "ipv4.csv",
...
SOURCE_URL_MODE_ENABLED = cfg["SOURCE_URL_MODE_ENABLED"]
SOURCE_TXT_MODE_ENABLED = cfg["SOURCE_TXT_MODE_ENABLED"]
SOURCE_TXT_FILE = cfg["SOURCE_TXT_FILE"]
SOURCE_CSV_MODE_ENABLED = cfg["SOURCE_CSV_MODE_ENABLED"]
SOURCE_CSV_FILE = cfg["SOURCE_CSV_FILE"]
...
def _parse_text_nodes(text):
    """从纯文本中提取标准节点（全新升级：支持任意格式混合、CSV、普通TXT，内置备用 API 查询）"""
    nodes = []
    pending = []
    ...
    for line in text.splitlines():
        ...
        port = "443"
        # 1. 尝试匹配 IP:PORT
        port_match = re.search(re.escape(ip) + r':(\d+)', line)
        if port_match:
            port = port_match.group(1)
        else:
            # 2. 尝试匹配 IP,PORT 或 IP PORT (用于兼容 CSV 及各类奇葩排版)
            csv_port_match = re.search(re.escape(ip) + r'[\s,;]+(\d{2,5})\b', line)
            if csv_port_match:
                port = csv_port_match.group(1)
        ...
        if hash_match:
            country = extract_country_code(hash_match.group(1))

        # 2. 如果没找到井号，遍历所在行的其余内容找寻可能的国家标识
        if not country:
            tokens = re.split(r'[\s,;|/]+', line)
            for token in tokens:
                if token in (ip, port):
                    continue
                code = extract_country_code(token)
                if code:
                    country = code
                    break
...
def main():
    ...
    # ==================== 三大模式自动混合读取 ====================
    # 模式 1: URL 优选模式
    if SOURCE_URL_MODE_ENABLED:
        print("\n[模式 1] 开始从 URL 数据源获取节点...")
    ...
    # 模式 2: ipv4.txt 模式
    if SOURCE_TXT_MODE_ENABLED:
        print(f"\n[模式 2] 开始从本地文本文件 ({SOURCE_TXT_FILE}) 获取节点...")
        if os.path.exists(SOURCE_TXT_FILE):
            try:
                with open(SOURCE_TXT_FILE, "r", encoding="utf-8") as f:
                    text = f.read()
                txt_nodes = parse_adaptive(text)
                print(f"从 {SOURCE_TXT_FILE} 解析出 {len(txt_nodes)} 个节点。")
                nodes.extend(txt_nodes)
            except Exception as e:
                print(f"❌ 读取 {SOURCE_TXT_FILE} 失败: {e}")
        else:
            print(f"⚠️ 未找到 {SOURCE_TXT_FILE}，跳过此模式。")

    # 模式 3: ipv4.csv 模式 (自适应支持任何带逗号分号等杂项的 CSV 格式)
    if SOURCE_CSV_MODE_ENABLED:
        print(f"\n[模式 3] 开始从本地 CSV 文件 ({SOURCE_CSV_FILE}) 获取节点...")
        if os.path.exists(SOURCE_CSV_FILE):
            try:
                with open(SOURCE_CSV_FILE, "r", encoding="utf-8") as f:
                    text = f.read()
                csv_nodes = parse_adaptive(text)
                print(f"从 {SOURCE_CSV_FILE} 解析出 {len(csv_nodes)} 个节点。")
                nodes.extend(csv_nodes)
            except Exception as e:
                print(f"❌ 读取 {SOURCE_CSV_FILE} 失败: {e}")
        else:
            print(f"⚠️ 未找到 {SOURCE_CSV_FILE}，跳过此模式。")
    ...
    nodes = unique_nodes
    print(f"\n合并去重后总计进入流水线 {len(nodes)} 个节点。")

    if not nodes:
        print("没有获取到任何有效节点，退出程序。")
        sys.exit(1)
