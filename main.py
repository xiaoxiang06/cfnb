...
支持 Windows / Linux
优化：增加交互式模式选择 (默认URL, txt, csv)，加入万能正则自适应解析器
"""
...
def _parse_text_nodes(text):
    """从纯文本中提取标准节点（支持任意格式混合、CSV、普通TXT，内置备用 API 查询）"""
    nodes = []
    pending = []
...
def select_modes_interactive():
    # 判断是否在非交互式终端中运行 (例如计划任务)
    if not sys.stdin.isatty():
        print("检测到非交互式环境运行，默认自动加载 URL 优选模式。")
        return True, False, False

    print("="*60)
    print("请选择节点获取来源 (可输入组合数字多选，直接回车则默认选1)：")
    print("  1. URL 优选 (从 config.json 配置中的 ADDITIONAL_SOURCES 获取)")
    print("  2. 读取本地 ipv4.txt 文件")
    print("  3. 读取本地 ipv4.csv 文件 (万能解析，支持各种混合/分隔符格式)")
    print("="*60)
    try:
        choice = input("请输入您的选择 [默认 1]: ").strip()
    except EOFError:
        choice = "1"
    ...
    mode_url = "1" in choice
    mode_txt = "2" in choice
    mode_csv = "3" in choice

    return mode_url, mode_txt, mode_csv
...
def main():
    ...
    # === 交互式模式选择 ===
    mode_url, mode_txt, mode_csv = select_modes_interactive()
    nodes = []
    ...
    if mode_txt:
        txt_file = "ipv4.txt"
        print(f"\n[模式 2] 开始从本地文本文件 ({txt_file}) 获取节点...")
        if os.path.exists(txt_file):
            try:
                with open(txt_file, "r", encoding="utf-8") as f:
                    text = f.read()
                txt_nodes = parse_adaptive(text)
                print(f"从 {txt_file} 解析出 {len(txt_nodes)} 个节点。")
                nodes.extend(txt_nodes)
            except Exception as e:
                print(f"❌ 读取 {txt_file} 失败: {e}")
        else:
            print(f"⚠️ 未找到 {txt_file}，跳过该来源。")

    # 3. 模式 CSV
    if mode_csv:
        csv_file = "ipv4.csv"
        print(f"\n[模式 3] 开始从本地 CSV 文件 ({csv_file}) 获取节点...")
        if os.path.exists(csv_file):
            try:
                with open(csv_file, "r", encoding="utf-8") as f:
                    text = f.read()
                csv_nodes = parse_adaptive(text)
                print(f"从 {csv_file} 解析出 {len(csv_nodes)} 个节点。")
                nodes.extend(csv_nodes)
            except Exception as e:
                print(f"❌ 读取 {csv_file} 失败: {e}")
        else:
            print(f"⚠️ 未找到 {csv_file}，跳过该来源。")
